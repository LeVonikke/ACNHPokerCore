        ...
        public bool Connect()
        {
            lock (_sync)
            {
                ...
                if (debugging)
                {
                    ...
                    {
                        MyMessageBox.Show("NO USB DEVICES FOUND!", "List of USB devices", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                        MyMessageBox.Show(deviceList, "List of USB devices", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }

                if (!CorrectIDFound && NormalIDFound)
                {
                    MyMessageBox.Show("Please double check your USB-botbase and libusbK driver installation!", "USB-botbase Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Connected = false;
                    return false;
                }
                ...
                if (CorrectIDFound)
                {
                    ...
                    {
                        MyMessageBox.Show("Device Not Found!\nPlease try restarting your Switch if problem persists!", "USB insertion always require at least 3 flips!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
