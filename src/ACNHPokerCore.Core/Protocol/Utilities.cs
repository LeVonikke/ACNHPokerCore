...
namespace ACNHPokerCore.Core
{
    public static class Utilities
    {
        ...
        // NOTE: original used @"csv\" etc. (Windows-only separator). Forward slash works
        // as a path separator on both Linux and Windows, so it's used here for portability.
        ...
        #region Emulator
        // Windows-only feature: reads/writes an emulator's process memory directly via
        // kernel32.dll (OpenProcess/ReadProcessMemory/WriteProcessMemory/VirtualQueryEx/
        // GetSystemInfo). This has no Linux equivalent and is out of scope for this port
        // (see ACNHPokerCore-Linux README). The P/Invoke declarations below still compile
        // fine on Linux; they simply throw at the first call (DllNotFoundException) because
        // isEmulator is never set to true by the Avalonia UI. Left in place instead of
        // deleted so a future session could implement a Linux/ptrace-based equivalent
        // behind the same isEmulator flag without having to reconstruct the call sites.
        ...
        public static byte[]? ReadEmulatorMemory(UInt32 Address, int size)
        {
            if (!OperatingSystem.IsWindows())
                throw new PlatformNotSupportedException("Emulator RAM reading uses kernel32.dll and is Windows-only; it is not available on Linux.");
        ...
        {
            if (!OperatingSystem.IsWindows())
                throw new PlatformNotSupportedException("Emulator RAM writing uses kernel32.dll and is Windows-only; it is not available on Linux.");
